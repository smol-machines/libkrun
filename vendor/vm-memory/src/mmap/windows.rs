...
pub struct MmapRegion<B> {
    ...
    /// True when `addr` is a `MapViewOfFile` view (file-backed or a CoW view from
    /// [`MmapRegion::build_raw`]) rather than a `VirtualAlloc` reservation, so
    /// `Drop` releases it with `UnmapViewOfFile` instead of `VirtualFree`.
    is_view: bool,
}
...
impl<B: NewBitmap> MmapRegion<B> {
    ...
    /// Wraps an already-mapped `MapViewOfFile` view (`addr`, `size` bytes) as an
    /// `MmapRegion`, taking ownership of it (`Drop` calls `UnmapViewOfFile`).
    ///
    /// libkrun uses this to wrap a `FILE_MAP_COPY` (copy-on-write) view of a
    /// frozen golden VM's guest-RAM file, giving a fork clone dense, page-shared
    /// guest memory. `prot`/`flags` are accepted for signature parity with the
    /// Unix `build_raw` and ignored on Windows.
    ///
    /// # Safety
    /// `addr` must point to a live mapped view of at least `size` bytes that is
    /// not owned by anything else; ownership transfers to the returned region.
    pub unsafe fn build_raw(addr: *mut u8, size: usize, _prot: i32, _flags: i32) -> io::Result<Self> {
        if addr.is_null() {
            return Err(io::Error::from_raw_os_error(libc::EINVAL));
        }
        Ok(Self {
...
impl<B> Drop for MmapRegion<B> {
    fn drop(&mut self) {
        // This is safe because we mapped/reserved the area at addr ourselves and
        // nobody else is holding a reference to it. A `MapViewOfFile` view (file-
        // backed or a `build_raw` CoW view) must be released with
        // `UnmapViewOfFile`; a `VirtualAlloc` reservation with `VirtualFree`
        // (size must be 0 with MEM_RELEASE, or the call fails).
        unsafe {
            let ret_val = if self.is_view {
