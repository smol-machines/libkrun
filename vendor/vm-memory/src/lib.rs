...
// This crate is vendored (see the workspace Cargo.toml [patch]) and, as a path
// dependency, is not lint-capped, so a newer rustc's stylistic lints would trip
// the workspace's `-D warnings`. Allow the ones the upstream 0.17.1 source pre-
// dates; they are not relevant to the libkrun patch (a Windows `build_raw`).
#![allow(unknown_lints)]
