...
/// Feeds host input into the guest's console-input virtqueue. Returns the
/// [`Queue`] when stopped (via `stop`) so the device can reclaim its virtqueue
/// indices for a checkpoint/restore at a clean boundary.
